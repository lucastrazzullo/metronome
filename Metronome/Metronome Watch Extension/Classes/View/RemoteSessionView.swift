...
struct RemoteSessionView: View {
    ...
    private func buildWaitingView() -> some View {
        return VStack {
            Text("Metronome").brandFont(.headline)
            Text("Waiting for iOS").brandFont(.body)
        }
    }
...
struct RemoteSessionView_Previews: PreviewProvider {

    static var previews: some View {
        let controller = RemoteSessionController()
        return RemoteSessionView(viewModel: RemoteSessionViewModel(controller: controller))
    }
}
